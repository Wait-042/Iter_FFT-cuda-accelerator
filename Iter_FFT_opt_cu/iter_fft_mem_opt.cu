...
//
//// 并查集节点，16字节对齐以满足GPU内存合并访问要求
//struct alignas(16) DSUNode {
//    int parent;   // 父节点索引
//    int offset;   // 到父节点的相对偏移量
//    int rank;     // 树的高度（按秩合并用）
//    int padding;  // 补齐16字节，避免内存访问浪费
//};
//
//struct FastDSU {
...
//    size_t cub_temp_storage_bytes = 32 * 1024 * 1024; // 扩展至 64MB 覆盖全图 CUB 临时空间
...
//    );
//
//    CUDA_CHECK(cudaStreamSynchronize(ws.streams[0]));
//
//    return d_pinv;
//}
//
...
//    int merged_count = 0;
//
//    // --- 优化点 1：早期退出上限 (Early Exit Bound) ---
//    // 对于常规网格图，N 个节点最多合并 N - 1 次；若有 Mask，此处可进一步传入有效像素点数
...
//        int r2 = p2;
//
//        // --- 优化点 2：分支预测优化 (Branch Prediction) ---
//        // 在 DSU 初期，大部分节点的 parent 都是自己 (p1 == parent)，直接命中可以跳过 find 函数
//        // 使用 __builtin_expect 告知编译器 parent != p1 是极小概率发生的“慢速分支”
//#if defined(__GNUC__) || defined(__clang__)
//        if (__builtin_expect(dsu_ptr[p1].parent != p1, 0)) {
//            r1 = dsu.find(p1, off1);
//        }
//        if (__builtin_expect(dsu_ptr[p2].parent != p2, 0)) {
//            r2 = dsu.find(p2, off2);
//        }
//#else
//        // MSVC 强力提示
//        if (dsu_ptr[p1].parent != p1) [[unlikely]] {
//            r1 = dsu.find(p1, off1);
//        }
//        if (dsu_ptr[p2].parent != p2) [[unlikely]] {
//            r2 = dsu.find(p2, off2);
//        }
//#endif
//
...
//    process_envelope_b_and_phi_kernel<T> << <blocksPerGrid, threadsPerBlock, 0, ws.streams[1] >> > (d_fft_bx, d_envelope_bx, d_phix, size, fft_scale);
//    process_envelope_b_and_phi_kernel<T> << <blocksPerGrid, threadsPerBlock, 0, ws.streams[2] >> > (d_fft_by, d_envelope_by, d_phiy, size, fft_scale);
//
